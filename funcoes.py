...
def cria_json(lista_hex, lista_mips,lista_regs,lista_stdout, nome):
    cur_path = os.path.dirname(__file__)
    lista_arquivos = [
        arquivo
    ...
    if len(lista_mips) > 0:
        ...
        if nome_arquivo in lista_arquivos:
            with open(caminho_arquivo, encoding="utf-8") as json_file_read:
                ...
                dictdict = novo_json(lista_hex, lista_mips,lista_regs,lista_stdout)
            ...
            with open(caminho_arquivo, "w", encoding="utf-8") as outfile:
                data = novo_json(lista_hex, lista_mips,lista_regs,lista_stdout)
